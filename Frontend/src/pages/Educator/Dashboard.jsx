...
function Dashboard() {
  ...
  const CourseProgressData = creatorCourseData?.map((course) => ({
    name: (course.title?.slice(0, 10) || "Course") + "...",
  ...
  // Avatar: show photo if available, else show initials div
  const avatarLetter = userData?.name?.slice(0, 1).toUpperCase() || "?";
      ...
      <div className='w-full px-6 py-10 bg-gray-50 space-y-10'>
        ...
        <div className='max-w-5xl mx-auto bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row items-center gap-6'>
          ...
          {userData?.photoUrl ? (
            ...
            <div className='w-28 h-28 rounded-full border-4 border-black shadow-md bg-black text-white flex items-center justify-center text-4xl font-bold'>
              {avatarLetter}
            </div>
          )}
